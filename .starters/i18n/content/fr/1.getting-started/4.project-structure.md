...
description: Comprendre la structure de projet de TockDocs v4.
navigation:
...
TockDocs est un **layer Nuxt** qui étend votre application Nuxt standard avec des fonctionnalités de documentation. Cela vous donne la flexibilité d'un projet Nuxt classique.

Lorsque vous créez un nouveau projet TockDocs avec `npx create-tockdocs my-docs`, voici la structure de base:
...
C'est ici que vous rédigez vos pages en Markdown. TockDocs génère automatiquement les routes basées sur votre structure de fichiers.
...
::tip{to="https://tockdocs.dev/concepts/internationalization"}
...
Ce fichier contient toutes les dépendances et scripts de votre application. Le `package.json` d'une application TockDocs est vraiment minimal et ressemble à :
...
_Ce fichier n'est pas obligatoire pour démarrer une application TockDocs._
...
C'est ici que vous pouvez configurer TockDocs pour l'adapter à votre marque, gérer le SEO, définir votre locale et adapter les liens et réseaux sociaux. Consultez la [documentation de configuration](https://tockdocs.dev/concepts/configuration) pour plus de détails.
...
Puisque TockDocs est un layer Nuxt, vous pouvez utiliser **n'importe quelle fonctionnalité** d'un projet Nuxt standard :
...
::tip{to="https://tockdocs.dev/concepts/nuxt"}
