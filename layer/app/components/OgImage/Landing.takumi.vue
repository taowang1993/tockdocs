...
import { findExistingPublicAsset } from '../../../utils/public-assets.server'
...
async function resolveLogoDataUrl(path?: string): Promise<string> {
  ...
  const localLogoDataUrl = await readLocalLogoDataUrl(path)
  if (localLogoDataUrl) {
...
async function readLocalLogoDataUrl(path: string): Promise<string> {
  if (path.startsWith('http')) {
    return ''
  }

  const candidate = await findExistingPublicAsset(path)
